...
class AddressVC: UIViewController {
    ...
    private let vnTxt = "Viet Nam"
    private var isVietNam = true
    
    var firstNameTxt = ""
    var lastNameTxt = ""
    var addressLine1Txt = ""
    var addressLine2Txt = ""
    var countryTxt = "Viet Nam"
    var stateTxt = ""
    var cityTxt = ""
    var zipcodeTxt = ""
    var phoneNumberTxt = ""
...
extension AddressVC {
    ...
    func fetchStatesData() { //Viet Nam
        states.removeAll()
        cities.removeAll()
        DispatchQueue.main.async {
            guard let path = Bundle.main.path(forResource: "cities-vn.json", ofType: nil)
                else { return }
            do {
                let data = try Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe)
                let result = try JSONSerialization.jsonObject(with: data, options: .mutableLeaves)
                ...
                if let dict = result as? [String : Any] {
                    for (_, value) in dict {
                        if let dict = value as? [String : Any],
                            let name = dict["name_with_type"] as? String {
                            if !self.states.contains(name) {
                                self.states.append(name)
                                self.states.sort(by: { $0 < $1 })
                            }
                            
                            //City
                            if let qhDict = dict["quan-huyen"] as? [String : Any] {
                                if name == self.stateTxt {
                                    for (_, value) in qhDict {
                                        if let dict = value as? [String : Any],
                                            let name = dict["name_with_type"] as? String {
                                            if !self.cities.contains(name) {
                                                self.cities.append(name)
                                                self.cities.sort(by: { $0 < $1 })
                                            }
                                        }
                                    }
                                }
                ...
                print("Load city error: \(error.localizedDescription)")
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if section == 0 {
            ...
            case 3:
                ...
                let isState = rTxt == "Tiểu Bang" ? true : false
                if isVietNam {
                    rTxt = isState ? "Tỉnh" : "State"
                    stateView = UIView()
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if textField == firstNameTF {
            ...
            if let text = zipcodeTF.text {
                if !text.trimmingCharacters(in: .whitespaces).isEmpty,
                    text.containsOnlyDigits, text.count < 9 {
...
extension AddressVC: PopUpViewDelegate {
    ...
    func fetchCountry(_ country: String, view: PopUpView) {
        ...
        isVietNam = country == vnTxt
        if !isVietNam { stateLbl.text = nil; cityLbl.text = nil; stateTxt = ""; cityTxt = "" }
