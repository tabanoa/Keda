...
class FeaturedDetailVC: UIViewController {
    
    //MARK: - Properties
    private let internetView = InternetView()
    private let collectionView = UICollectionView(frame: .zero, collectionViewLayout: UICollectionViewFlowLayout())
    private let naContainerV = UIView()
    private let featuredDetailLayout = FeaturedDetailLayout()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        guard !appDl.isFeaturedDetailVC else { return }
        naContainerV.isHidden = true
        collectionView.isHidden = true
        let hud = createdHUD()
...
extension FeaturedDetailVC {
    ...
    func setupCV() {
        ...
        collectionView.register(FeaturedDetailCVCell.self, forCellWithReuseIdentifier: FeaturedDetailCVCell.identifier)
        
        collectionView.collectionViewLayout = featuredDetailLayout
        featuredDetailLayout.contentPadding = SpacingMode(horizontal: 2.0, vertical: 2.0)
        featuredDetailLayout.cellPadding = 2.0
...
extension FeaturedDetailVC: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FeaturedDetailCVCell.identifier, for: indexPath) as! FeaturedDetailCVCell
...
extension FeaturedDetailVC: UICollectionViewDelegate {

    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let cell = collectionView.cellForItem(at: indexPath) as! FeaturedDetailCVCell
        let featured = self.featured[indexPath.item]
...
extension FeaturedDetailVC: UICollectionViewDelegateFlowLayout {
...
extension FeaturedDetailVC: UITextFieldDelegate {
    
    func textFieldDidBeginEditing(_ textField: UITextField) {
        textField.resignFirstResponder()
        
        let searchVC = SearchVC()
        searchVC.products = featured
...
extension FeaturedDetailVC: UINavigationControllerDelegate {
...
extension FeaturedDetailVC: FilteredVCDelegate {
    
    func handleDoneDidTap(_ color: String, _ rating: Int, vc: FilterVC) {
        vc.handlePop()
        
        Product.filterByColorAndRating(color, rating, prs: featured) { (prs) in
    ...
    func handleDoneDidTap(_ color: String, vc: FilterVC) {
        ...
        Product.filterByColor(color, prs: featured) { (prs) in
            self.handlePushFilteredVC(prs)
        }
    }
    ...
    func handleDoneDidTap(_ rating: Int, vc: FilterVC) {
        ...
        Product.filterByRating(rating, prs: featured) { (prs) in
            self.handlePushFilteredVC(prs)
        }
    }
...
extension FeaturedDetailVC: InternetViewDelegate {
