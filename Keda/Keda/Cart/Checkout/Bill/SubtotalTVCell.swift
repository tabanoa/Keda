//
//  SubtotalTVCell.swift
//  Keda
//
//  Created by Matthew Mukherjee on 03/03/2021.
import UIKit

class SubtotalTVCell: UITableViewCell {
    
    //MARK: - Properties
    static let identifier = "SubtotalTVCell"
...
extension SubtotalTVCell {
    ...
    func configureCell() {
        ...
        //TODO: - NameLbl
        let subtotalNameTxt = NSLocalizedString("SUBTOTAL", comment: "SubtotalTVCell.swift: SUBTOTAL")
        subtotalTitleLbl.configureNameForCell(false, txtColor: .darkGray, fontSize: 15.0, isTxt: subtotalNameTxt, fontN: fontNamedBold)
        
        let feeNameTxt = NSLocalizedString("SHIPPING FEE", comment: "SubtotalTVCell.swift: SHIPPING FEE")
        shippingFeeTitleLbl.configureNameForCell(false, txtColor: .darkGray, fontSize: 15.0, isTxt: feeNameTxt, fontN: fontNamedBold)
        
        let taxNameTxt = NSLocalizedString("INSURANCE FEE", comment: "SubtotalTVCell.swift: INSURANCE FEE")
...
extension SubtotalTVCell {
    ...
    private func setupDarkModeView(_ isDarkMode: Bool = false) {
        ...
        subtotalTitleLbl.textColor = isDarkMode ? .lightGray : .darkGray
        subtotalLbl.textColor = isDarkMode ? .lightGray : .darkGray
        
        shippingFeeTitleLbl.textColor = isDarkMode ? .lightGray : .darkGray
        shippingFeeLbl.textColor = isDarkMode ? .lightGray : .darkGray
        
        taxTitleLbl.textColor = isDarkMode ? .lightGray : .darkGray
        taxLbl.textColor = isDarkMode ? .lightGray : .darkGray
    }
}
